---
title: "Detection of differentially expressed miRNAs between BRAFV600E and RAS samples (tumor-normal paired samples only) using DESeq2"
author: "Jieqiong Dai"
...
# Reads in microRNA reads count data
```{r,message=FALSE,warning=FALSE}
library(DESeq2)
rna <- read.csv("phase1234_miRNA_reads_count_ready.csv",header=T,row.names=1)
```
...
# Detect BRAFV600E M vs RAS M DE miRNAs using tumor-normal paired samples
```{r,message=FALSE,warning=FALSE}
sample2 <- read.csv("paired_miRNA_sample_table.csv",header=T,row.names=1)
...
nrow(res_fc1.5q0.05)
write.csv(res_fc1.5q0.05,"miRNA_BRAFmutationVSRASmutation_fc1.5q0.05.csv")

```

# Get vst data of top BRAFV600E M vs RAS M DE miRNAs (fold change >=1.5, p-adjust <=0.05) in tumor samples
```{r,message=FALSE,warning=FALSE,results='hide'}
sample_tu <- subset(sample,Specimen.Type.ID == "Primary Tumor")
vst_tu <- vst[,colnames(vst)%in%rownames(sample_tu)]
top100_vst_tu <- assay(vst)[rownames(vst)%in%rownames(res_fc1.5q0.05[1:min(nrow(res_fc1.5q0.05),100),]),colnames(vst)%in%rownames(sample_tu)]
write.csv(top100_vst_tu,"miRNA_top_BRAFvsRAS_vst_inTumor.csv")
...
library(pheatmap)
library("vsn")
plotPCA(vst, intgroup=c("Specimen.Type.ID","FinalDriverPathway.10Jan2020")) + ggtitle("miRNA PCA, all samples")
library(plotly)
...
# Heamap of vst data of the top BRAFV600E M vs RAS M DE miRNAs in tumor samples
```{r,message=FALSE,warning=FALSE}
...
vst2 <- vst(dds2, blind=F,nsub=300)
...
```{r, message=FALSE,warning=FALSE}
plotPCA(vst2, intgroup=c("Specimen.Type.ID","driver_group.1")) + ggtitle("miRNA PCA, tumor normal paired samples")
...
```
## Dispersion, plot, with the final estimates shrunk from the gene-wise estimates towards the fitted estimates. Some gene-wise estimates are flagged as outliers and not shrunk towards the fitted value.The amount of shrinkage depends on the sample size, the number of coefficients, the row mean and the variability of the gene-wise estimates.
```{r, message=FALSE,warning=FALSE}
plotDispEsts(dds2)

save.image("miRNA_BRS_DE.Rdata")
```
