...
library(pheatmap)
library("vsn")
plotPCA(vst, intgroup=c("Specimen.Type.ID","FinalDriverPathway.10Jan2020")) + ggtitle("totalRNA driver PCA, all counts")
library(plotly)
...
```{r, message=FALSE,warning=FALSE}
plotPCA(vst2, intgroup=c("Specimen.Type.ID","driver_group.1")) + ggtitle("paired_sample_totalRNA_driver_PCA, all counts")
...
plotDispEsts(dds2)
```
# Heatmap of sample to sample distance, an overview over similarities and dissimilarities between samples using transformed count matrix of all samples.
```{r, message=FALSE,warning=FALSE}
sampleDists <- dist(t(assay(vst2)))
sampleDistMatrix <- as.matrix(sampleDists)
rownames(sampleDistMatrix) <- paste(vst2 $ Specimen.Type.ID, vst2 $ TSS.Subject.ID, sep = "-")
